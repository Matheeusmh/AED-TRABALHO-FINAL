...
int removerCidade(descritorRodovias *descritor, char *cidade){
    ...
    while (aux != NULL) {
        ...
        while (cityAtual != NULL) {
            ...
            if (strcmp(cityAtual->cidade, cidade) == 0){
                ...
                novaCidade cityRemove = cityAtual; // Variável para armazenar a cidade que será removida enquanto o loop for executado.
                cityAtual = cityAtual->prox; // Avança para a próxima cidade da lista.
                free(cityRemove); // Liberando a memória da cidade a ser removida...
            }
    ...
    // Verificando se a cidade está ou não.
    if(!achou)
        return 0;
    else
        return 1;
    ...
    while (1) {
        ...
        switch (escolha) {
            ...
            case 2:
                ...
                if(removerCidade(rodovias, nomeCidade))
                    printf("\nA cidade %s foi removida!\n", nomeCidade);
                else
                    printf("\nA cidade %s não foi encontrada na lista de cidades!\n", nomeCidade);
            ...
            case 3:
                ...
                if(removerRodovia(rodovias, nomeRodovia))
                    printf("\nA rodovia %s foi removida!\n", nomeRodovia);
                else
                    printf("\nA rodovia %s não foi encontrada na lista de rodovias!\n", nomeRodovia);
